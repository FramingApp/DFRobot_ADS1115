...
import smbus
import time
#from BitBangI2C_script import BitBangI2C
...
class ADS1115():
    ...
    #     self.SDA = 19
    #     self.SCL = 26  # Fix this line

    #     self.pi.bb_i2c_open(self.SDA, self.SCL, 100000)
    ...
    # def bb_write_i2c_block_data(self, addr, reg_addr, data):
    #     start = [4, addr]  # START, I2C address
    #     write = [2]  # WRITE mode
    #     stop = [3]  # STOP
    
    #     cmd = start + write + [1, reg_addr] + write + [len(data)] + data + stop
    #     self.pi.bb_i2c_zip(self.SDA, cmd)
 
    # def bb_read_i2c_block_data(self, addr, reg_addr, length):
    #     start = [4, addr]  # START, I2C address
    #     write = [2]  # WRITE mode
    #     read = [1, length]  # READ mode with length bytes
    #     stop = [3]  # STOP
    
    #     cmd = start + write + [1, reg_addr] + start + write + read + stop
    #     count, data = self.pi.bb_i2c_zip(self.SDA, cmd)
    #     return list(data)
    ...
    def set_single(self):
        ...
        global addr_G
        if self.channel == 0:
            CONFIG_REG = [ADS1115_REG_CONFIG_OS_SINGLE | ADS1115_REG_CONFIG_MUX_SINGLE_0 | mygain | ADS1115_REG_CONFIG_MODE_CONTIN, ADS1115_REG_CONFIG_DR_860SPS | ADS1115_REG_CONFIG_CQUE_NONE]
        elif self.channel == 1:
            CONFIG_REG = [ADS1115_REG_CONFIG_OS_SINGLE | ADS1115_REG_CONFIG_MUX_SINGLE_1 | mygain | ADS1115_REG_CONFIG_MODE_CONTIN, ADS1115_REG_CONFIG_DR_860SPS | ADS1115_REG_CONFIG_CQUE_NONE]
        elif self.channel == 2:
            CONFIG_REG = [ADS1115_REG_CONFIG_OS_SINGLE | ADS1115_REG_CONFIG_MUX_SINGLE_2 | mygain | ADS1115_REG_CONFIG_MODE_CONTIN, ADS1115_REG_CONFIG_DR_860SPS | ADS1115_REG_CONFIG_CQUE_NONE]
        elif self.channel == 3:
            CONFIG_REG = [ADS1115_REG_CONFIG_OS_SINGLE | ADS1115_REG_CONFIG_MUX_SINGLE_3 | mygain | ADS1115_REG_CONFIG_MODE_CONTIN, ADS1115_REG_CONFIG_DR_860SPS | ADS1115_REG_CONFIG_CQUE_NONE]
        
        bus.write_i2c_block_data(addr_G, ADS1115_REG_POINTER_CONFIG, CONFIG_REG)
        #self.bb_write_i2c_block_data(addr_G, ADS1115_REG_POINTER_CONFIG, CONFIG_REG)
    ...
    def read_value(self):
        ...
        #data = self.bb_read_i2c_block_data(addr_G, ADS1115_REG_POINTER_CONVERT, 2)
        ...
        if raw_adc > 32767:
            raw_adc -= 65535
        code = raw_adc
        raw_adc = float(raw_adc)*coefficient
        return {'r' : raw_adc, 'c' : code}
